...
<div class="mb-6">
    <h1 class="text-3xl font-bold mb-2">
        <i class="fas fa-shield-alt text-green-500 mr-3"></i>Manage Teams
    </h1>
...
<div class="bg-dark-card border border-dark-border rounded-lg p-6 mb-6">
    <h2 class="text-xl font-bold mb-6"><?php echo $action == 'edit' ? 'Edit' : 'Add New'; ?> Team</h2>
    <form method="POST" action="">
        ...
        <div class="mb-6">
            ...
            <input type="url" name="logo_url"
                value="<?php echo $edit_team ? htmlspecialchars($edit_team['logo_url']) : ''; ?>"
                placeholder="https://example.com/logo.png"
                class="w-full bg-dark-bg border border-dark-border rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500">
            <p class="text-xs text-gray-400 mt-2">Enter the full URL of the team logo image</p>
        </div>
        
        <div class="flex space-x-3">
            <button type="submit" name="<?php echo $action == 'edit' ? 'update_team' : 'add_team'; ?>"
                class="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-lg transition">
                <i class="fas fa-save mr-2"></i><?php echo $action == 'edit' ? 'Update' : 'Add'; ?> Team
            </button>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php
    $teams = $conn->query("SELECT t.*, COUNT(p.id) as player_count FROM teams t LEFT JOIN players p ON t.id = p.team_id GROUP BY t.id ORDER BY t.name");
    if ($teams->num_rows > 0):
    ...
    <div class="bg-dark-card border border-dark-border rounded-lg p-6">
        <div class="text-center mb-4">
            ...
            <img src="<?php echo htmlspecialchars($team['logo_url']); ?>" alt="" class="w-20 h-20 object-contain mx-auto mb-3">
            <?php else: ?>
            <div class="w-20 h-20 bg-gradient-to-br from-green-500 to-blue-600 rounded-full flex items-center justify-center mx-auto mb-3">
                <span class="text-2xl font-bold"><?php echo strtoupper(substr($team['name'], 0, 2)); ?></span>
            </div>
            <?php endif; ?>
            <h3 class="text-lg font-bold mb-2"><?php echo htmlspecialchars($team['name']); ?></h3>
            <p class="text-sm text-gray-400">
                <i class="fas fa-users mr-1"></i><?php echo $team['player_count']; ?> Players
            </p>
        </div>
        <div class="flex space-x-2">
            <a href="?action=edit&id=<?php echo $team['id']; ?>" 
               class="flex-1 bg-yellow-600 hover:bg-yellow-700 text-white text-center py-2 rounded transition">
                <i class="fas fa-edit"></i> Edit
            </a>
            <form method="POST" class="flex-1" onsubmit="return confirm('Delete this team?');">
    ...
    ?>
    <div class="col-span-3 bg-dark-card border border-dark-border rounded-lg p-8 text-center">
        <i class="fas fa-shield-alt text-4xl text-gray-600 mb-4 block"></i>
        <p class="text-gray-400">No teams found. Click "Add New Team" to create one.</p>
    </div>
    <?php endif; ?>
</div>
<?php endif; ?>
